 ...
 * @author red
 */
public class InventoryUpdateQuery implements Query<Event> {
	...
	public InventoryUpdateQuery(Player player, Item item, int slot) {
		this.player = player;
		this.item = item;
		this.slot = slot;
	}
	...
	public Event execute(Connection connection, SQLHandler handler) throws SQLException {
		if (item != null) {
			PreparedStatement statement = handler.getQuery("inventory-insert");
			...
			statement = handler.getQuery("inventory-update");
			statement.setInt(1, item.getId());
			statement.setInt(2, item.getAmount());
			statement.setInt(3, player.getUid());
			statement.setInt(4, slot);
			...
			PreparedStatement statement = handler.getQuery("inventory-delete");
