...
public class WalkingQueue {
	...
	public void processNextMovement() {
		...
		if (regionUpdateRequired) {
			...
			character.setLastUpdatedRegion(Main.getRealm().get(character));
		}
		
		/*
		 * If the entity is outside its region, remove it from the current region and update
		 * it accordingly
		 */
		if (!character.getRegion().equals(Main.getRealm().get(character))) {
			/*
			 * Get the new region
			 */
			Region region = Main.getRealm().get(character);
			
			/*
			 * If the character is player, do cool stuff
			 */
			if (character instanceof Player) {
				/*
				 * Remove the player from its current region
				 */
				character.getRegion().remove((Player) character);
				
				/*
				 * If there are no more players in the region the player has just left, destroy it
				 */
				if (character.getRegion().size() <= 0) {
					character.getRegion().destroy();
				}
				
				/*
				 * register the player
				 */
				region.register((Player) character);
			}
			
			/*
			 * Update the player's region
			 */
