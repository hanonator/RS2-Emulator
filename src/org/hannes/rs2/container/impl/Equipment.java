...
import org.hannes.rs2.util.ClientConfig;
import org.hannes.rs2.util.ItemDefinition;
import org.hannes.rs2.util.ItemModel;
import org.hannes.rs2.util.Label;
import org.hannes.rs2.util.SidebarInterface;
import org.hannes.sql.impl.EquipmentUpdateQuery;
...
public class Equipment extends Container implements Observer<ContainerEvent> {
	...
	public void set(int slot, Item item) {
		 ...
		 * Update db
		 */
		Main.getCharacterdatabase().submit(new EquipmentUpdateQuery(player, item, slot));
	...
	public void equip(EquipEvent event) throws Exception {
		try {
			...
			if (slot >= 0 && slot < player.getInventory().size() && player.getInventory().get(slot) != null) {
				 ...
				 */
				if (target == -1) {
					player.getConnection().write("I can't wear that.");
					return;
				}
