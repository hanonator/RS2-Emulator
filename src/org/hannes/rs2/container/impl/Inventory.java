...
import org.hannes.sql.impl.InventoryUpdateQuery;
...
public class Inventory extends Container implements Observer<ContainerEvent> {
	...
	public synchronized void set(int slot, Item item) {
		super.set(slot, item);
		Main.getCharacterdatabase().submit(new InventoryUpdateQuery(player, item, slot));
	}
	...
	public void reset() {
		...
		player.getConnection().write(new ContainerContext(this, Bank.INTERFACE_ID_INVENTORY));
	}
